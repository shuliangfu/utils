...
- **条件验证**：
  - 根据其他字段值进行条件验证（`when`）
- **错误消息定制**：
  - 支持 `ValidateOptions`（如 `options.messages`）按次覆盖；不传 options
    时使用默认英文
  - schema 上 `.message(rule, msg)` 按规则定制
  - 支持国际化
- **异步验证支持**：
  - 异步验证（`validateAsync`）
...
客户端版本的所有 API 与服务端版本完全相同（含 `ValidateOptions`
与自定义消息），请参考 [服务端文档](../validator.md)。
...
- **纯函数**：所有验证器都是纯函数，无副作用
- **错误消息**：支持 `options.messages` 与 `.message(rule, msg)`，不传 options
  时默认英文，支持国际化
